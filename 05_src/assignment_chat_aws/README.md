...
*▶ Click the image above to watch the demo on YouTube. The scenarios demonstrated in the video follow the [Test Plan](../assignment_chat/TEST_PLAN.md).*
...
| Item | Detail |
| --- | --- |
| Framework | Gradio 6.x `gr.Blocks` |
| Theme | `gr.themes.Soft()` (passed to `launch()` for Gradio 6 compatibility) |
...
This version (`assignment_chat_aws`) is the production-deployed variant running at **[https://dsi.fantinatti.net/](https://dsi.fantinatti.net/)**.

### Infrastructure

| Layer | Technology | Detail |
| --- | --- | --- |
| Cloud provider | AWS EC2 | Ubuntu 22.04 LTS, `t3.small` (2 vCPU, 2 GB RAM) |
| Public IP | Static Elastic IP | `98.91.246.71` — persists across instance stop/start |
| Domain | `fantinatti.net` (personal domain) | `dsi` A-record pointing to EC2 Elastic IP |
| Reverse proxy | Nginx | Terminates TLS, forwards to Gradio on `127.0.0.1:7860` |
| TLS certificate | Let's Encrypt (Certbot) | Free 90-day cert, auto-renewed via Certbot systemd timer |
| Access control | Nginx HTTP Basic Auth (`htpasswd`) | Bcrypt-hashed credentials in `/etc/nginx/.htpasswd`; checked on every request with no Nginx reload required for user changes |
| Process manager | systemd (`cosmos.service`) | Starts `app.py` automatically on boot, restarts on crash (`Restart=always`) |
| Python environment | `.venv` virtualenv | Isolated from system Python; activated via `ExecStart` absolute path |
| Secrets management | `.secrets` file (not committed) | Loaded by `EnvironmentFile=` in the systemd unit; keeps API keys out of the codebase |

### Nginx Configuration Highlights

| Directive | Value | Why |
| --- | --- | --- |
| `proxy_buffering off` | on | Disables response buffering — required for Gradio's SSE-based job queue; without this the UI loads but is completely unresponsive |
| `proxy_cache off` | on | Prevents Nginx from caching streaming responses |
| `map $http_upgrade $connection_upgrade` | `default upgrade` / `'' close` | Correctly sets `Connection` header for both WebSocket upgrade requests and normal HTTP requests |
| `proxy_set_header X-Forwarded-Proto $scheme` | `https` | Tells Gradio it is behind HTTPS so it generates correct absolute URLs |
| `proxy_read_timeout` / `proxy_send_timeout` | 300 s | Prevents Nginx from closing long-running LLM inference requests |

### Deployment Workflow

```bash
# 1. Sync code from Mac to EC2 (excludes .venv and generated vector DB)
rsync -av --exclude '.venv' --exclude 'data/chroma_db' \
  /Users/efantinatti/Downloads/deploying-ai/ \
  ubuntu@98.91.246.71:~/deploying-ai/

# 2. Restart the service after code changes
ssh ubuntu@98.91.246.71 'sudo systemctl restart cosmos'

# 3. Check live logs
ssh ubuntu@98.91.246.71 'sudo journalctl -u cosmos -f'
```

### Design Decisions — AWS-specific

- **Gradio `server_name="0.0.0.0"`** — Required so Gradio binds to all network interfaces, not just `localhost`, allowing Nginx to reach it via `127.0.0.1:7860`.
- **Gradio 6.x compatibility** — `theme` and `css` moved from `gr.Blocks()` constructor to `launch()`, and `type="messages"` removed from `gr.ChatInterface` (now the default), to match the Gradio version installed on EC2.
- **Basic Auth over app-level auth** — Nginx HTTP Basic Auth was chosen for simplicity: no code changes to the Gradio app are needed, credentials are managed with a single shell command (`htpasswd`), and it sits outside the application layer entirely.
- **systemd over screen/tmux** — systemd is the standard Linux init system; it handles auto-start on reboot, crash recovery, and structured log collection (`journald`) with no additional tooling.
